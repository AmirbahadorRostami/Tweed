...
public class RoomManager : MonoBehaviour
{
    ...
    private void UpdateRoom()
    {
        ...
        if (!inTrimTask && (plant.healthBar > (plant.maxHealthBar * plant.stateUpgradeThresh)) 
            && plant.state == PlantManager.State.vegetative )
        {
            if(!plant.isTrimmed && !trimTaskInitlized)
            {
                Task tt = new Task();
        ...
        if (!inCropTask && GM.level == 3 && plant.state == PlantManager.State.ready && !croppTaskInitlized && !plant.isCropped)
        {
    ...
    public void startWateringTask()
    {
        ...
        if (!GM.inTransition)
        {
            ...
            taskList.removeTask(this.plant.name, TaskType.water);
    ...
    public void startLightTask()
    {
        ...
        {
            // remove task from UI list
            taskList.removeTask(this.plant.name, TaskType.light);
    ...
    public void startNutriTask()
    {
        ...
        {
            // remove task from UI list
            taskList.removeTask(this.plant.name, TaskType.nutrition);
    ...
    public void startCropTask()
    {
        ...
        {
            // remove task from UI list
            taskList.removeTask(this.plant.name, TaskType.crop);
    ...
    public void startTrimTask()
    {
        ...
        {
            // remove task from UI list
            taskList.removeTask(this.plant.name, TaskType.trim);
